...
export class ProviderListComponent implements OnInit {
  ...
  loadData() {
    ...
    const totalPages = Math.ceil(
      this.filteredProviders.length / this.itemsPerPage
    );
  ...
  searchProvider($event) {
    ...
    if (value !== null && value !== undefined && value !== "") {
      this.filteredProviders = this.listProviders.filter(
        (c) =>
          c.name_provider.toLowerCase().indexOf(value.toLowerCase()) !== -1 ||
          this.changeProviderStateDescription(c.state_provider)
            .toLowerCase()
