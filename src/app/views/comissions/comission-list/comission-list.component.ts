...
export class ComissionListComponent implements OnInit {
      ...
      this._comissionsService.getAllComsDetail().subscribe((details: any[]) => {
        ...
        this.listComissions.forEach((comission) => {
          const detail = details.find(
            (detail) =>
              detail.id_commission_detail === comission.id_commission_detail
          );
          if (detail) {
  ...
  handleMonth(event: any) {
    ...
    this.new_comissionDetail.month_commission = `${currentYear}-${selectedMonth
      .toString()
      .padStart(2, "0")}-01`;
    console.log(this.new_comissionDetail.month_commission);
  }
  createComissionDetail() {
    this._comssionDetailService
      .createDetailCom(this.new_comissionDetail)
      .subscribe(
        (data) => {
          console.log(data);
          this.loading = true;
          setTimeout(() => {
            this.loading = false;
            this.toastr.success(
              "Detalle comisión creado con éxito.",
              "Proceso Completado",
        ...
        (error) => {
          this.loading = false;
          this.toastr.error("Ya existe un registro para este mes", "Error", {
            progressBar: true,
          });
  ...
  submit() {
    ...
    this.new_comissionDetail.commission_percentage =
  ...
  filterComissionsByMonth() {
    const currentYear = new Date().getFullYear();
    const selectedDate = `${currentYear}-${this.selectedMonth
      .toString()
      .padStart(2, "0")}-01`;
    const selectedDetail = this.details.find(
      (detail) => detail.month_commission === selectedDate
    );

    if (selectedDetail) {
      this.listComissions = this.originalListComissions.filter(
        (comission) =>
          comission.id_commission_detail === selectedDetail.id_commission_detail
      );
  ...
  updateListComissions() {
    ...
    const totalPages = Math.ceil(
      this.listComissions.length / this.itemsPerPage
    );
                ...
                this._comssionDetailService
                  ...
                  .subscribe(
                    ...
                    (error) => {
                      this.loading = false;
                      this.toastr.error(
                        "Error al asignar el porcentaje.",
                        "Error",
                        { progressBar: true, timeOut: 2000 }
                      );
                    }
