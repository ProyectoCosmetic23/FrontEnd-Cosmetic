...
export class ComissionsDetailComponent implements OnInit {
    ...
    forkJoin({
      ...
      (data: any) => {
        ...
        data.comissionDetail.sort((a, b) => {
          return (
            new Date(a.month_commission).getTime() -
            new Date(b.month_commission).getTime()
          );
        });
      ...
      this._comissionsService.getComsById(comissionId).subscribe(
        (data) => {
          ...
          if (this.comission && this.comission.comissions) {
            const idComissionDetail =
              this.comission.comissions.id_commission_detail;
  ...
  loadEmployees() {
    this._comissionsService.getAllEmployees().subscribe(
      (data) => {
        ...
        this.activeEmployees = this.listEmployees.filter(
  ...
  textComChange() {
    const selectedCommission = this.formBasic.get(
      ...
      .subscribe(
        (data) => {
          ...
          this.totalCommissions =
            this.totalSale * (this.commissionPercentage / 100);
          ...
          this.ngZone.run(() => {
            this.formBasic
              ...
              ?.patchValue(
                "$ " +
                  this.totalSale.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, "$&,")
              );
            this.formBasic
          ...
          if (this.totalSale == 0) {
            ...
            this.message =
              "El empleado no realizó o no ha realizado ventas para el mes elegido";
  ...
  findComsData(idComissionDetail: number, idEmployee: number) {
    ...
    const detail = this.listComisionDetail.find(
      (detail) => detail.id_commission_detail === idComissionDetail
    );
    const employee = this.listEmployees.find(
      (employee) => employee.id_employee === idEmployee
    );
    if (detail && employee) {
  ...
  updateCommissionPercentage() {
    ...
    const selectedCommission = this.listComisionDetail.find(
      (commission) => commission.id_commission_detail === selectedId
    );
    if (selectedCommission) {
      ...
      this.formBasic
        ...
        ?.setValue(selectedCommission.commission_percentage + " %");
    } else {
  ...
  handleNameProviderSelection(event: any) {
    ...
    const selectedCommission = this.listComisionDetail.find(
      (commission) =>
        commission.month_commission === this.comission.month_commission
    );
    if (selectedCommission) {
      this.comission.commission_percentage =
