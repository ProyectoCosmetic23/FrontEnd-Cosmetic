...
export class PurchaseDetailComponent implements OnInit {
  ...
  private inicializateForm(id: number): void {
    ...
    this.purchaseForm.get('invoice_number').setValidators([Validators.required])
  ...
  private inicializateProductForm(id: number): void {
    this.productForm = this.formBuilder.group({
      id_category: ['', [Validators.required,]],
      name_product: ['', [Validators.required, Validators.maxLength(80), Validators.pattern('^[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ ]+$'),]],
    ...
    this.productForm.get('max_stock').setValidators([Validators.required, this.validateStockMax.bind(this)]);
  }
  ...
  private createDetailFormGroup(detail: Detail): FormGroup {
    return this.formBuilder.group({
      ...
      name_product: [this.getProductName(detail.id_category, detail.id_product)],
  ...
  private setProductDetail() {
    const purchaseDetailFormArray = this.formBuilder.array(this.purchaseDetailArray.map(
      detail => this.createDetailFormGroup(detail))
    );
...
calculateTotal(){
  ...
  validateProductExist() {
    ...
    return new Promise((resolve) => {
      if (!product_name && !category_id) {
        ...
        this.productsService.getValidateProductExist(category_id, product_name).subscribe(
          (isAvailable) => {
  ...
  openCreateProductModal(content) {
    this.inicializateProductForm(Number(this.id));
    this.modalService.open(content, { ariaLabelledBy: 'createProductModal' }).result.then(
  ...
  getProductName(id_category: string, id_product: string): string {
    return this.listProducts.find(x => x.id_category == id_category && x.id_product == id_product).name_product
  }
